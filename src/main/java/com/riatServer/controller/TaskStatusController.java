...
public class TaskStatusController {
    ...
    @Autowired
    public TaskStatusController(TaskStatusRepo taskStatussRepo)
    {
        this.taskStatusRepo = taskStatussRepo;
    }

    @ApiOperation(value = "Получения списка всех отделов")
    @GetMapping
    public ResponseEntity<List<TaskStatus>> List(){
        List<TaskStatus> taskStatuss = taskStatusRepo.findAll();
        if(taskStatuss.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<>(taskStatuss, HttpStatus.OK);
    }
